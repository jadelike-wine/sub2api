export default {
  imageCredentials: {
    ...
    test: {
      ...
      reason: '失败原因',
      reasonSuccess: '成功',
      reasonDecryptFailed: '本地解密失败（密钥不匹配或密文损坏，未调用上游）',
      reasonAuthFailed: '上游认证失败（401，API Key 无效）',
      reasonForbidden: '上游禁止访问（403，Key 已被禁用）',
      reasonRateLimited: '上游限流（429，Key 本身有效）',
      reasonTimeout: '上游响应超时（Key 本身有效）',
      reasonUpstreamError: '上游错误（5xx/网络，Key 本身有效）',
      retry: '重新测试',
      close: '关闭'
    },
    // 存储状态
