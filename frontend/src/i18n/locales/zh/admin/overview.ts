export default {
    ...
    backup: {
      ...
      s3: {
        ...
        descriptionSuffix: '）',
        sharedUsageNote: '该配置同时用于 Agnes 多模态图片存储和数据库备份。Agnes 图片使用 agnes-chat/ 前缀，数据库备份使用 backups/ 前缀。',
        enabled: '启用 S3 存储',
        endpoint: '端点地址',
        region: '区域',
        bucket: '存储桶',
        prefix: 'Key 前缀',
        prefixFixedHint: '固定为 backups/，不可修改。与图片前缀（agnes-chat/、image-generation/）严格隔离。',
        accessKeyId: 'Access Key ID',
        secretAccessKey: 'Secret Access Key',
        secretConfigured: '已配置，留空保持不变',
        forcePathStyle: '强制路径风格',
        publicBaseUrl: 'Public Base URL',
        publicBaseUrlHint: '填写公开桶 / CDN 域名（如 https://cdn.example.com）时，Agnes 图片将返回永久直链；留空则返回带过期时间的 presigned 链接。数据库备份不使用此字段。',
        publicBaseUrlSecurityWarning: '安全警告：R2 自定义域名 / 公开桶通常映射整个 bucket。若该域名允许匿名读取 backups/ 前缀，所有数据库备份将对外公开泄露（备份路径已含随机段使批量扫描不可行，但单文件仍可被直接访问）。必须在 Cloudflare Worker / CDN 鉴权层仅放行 agnes-chat/ 与 image-generation/ 前缀，并显式拒绝 backups/；或保持 bucket 私有并使用 presigned 链接。UI 警告不是安全边界，CDN 层防护是必须的。',
        additionalPublicBaseUrls: '其他公开入口（Additional Public Base URLs）',
        additionalPublicBaseUrlsPlaceholder: 'https://pub-xxx.r2.dev\nhttps://cdn2.example.com',
        additionalPublicBaseUrlsHint: '若该 bucket 启用了 r2.dev Public Development URL 或其他 custom domain，请逐行列出。保存时后端会逐一探测确认它们同样拒绝 backups/ 匿名读取。推荐做法：在 Cloudflare 控制台禁用这些入口（此字段留空）。',
        bucketPrivacyAttestedLabel: '我已确认 bucket 已私有化（必填）',
        bucketPrivacyAttestedHint: '勾选即表示承诺：已禁用 R2 Public Development URL（*.r2.dev）；已移除或 Worker-protect 所有未受保护的 custom domain；bucket 未启用任何公开读策略；上方"其他公开入口"已完整列出所有剩余公开入口。后端无法直接读取 Cloudflare 控制台配置，此承诺是 HARD 前提；建议同时运行 deploy/cloudflare-worker/verify-policy.mjs 通过 Cloudflare API 权威验证。',
        bucketPrivacyAttestedRequired: '设置 Public Base URL 时必须勾选此承诺才能保存。',
        testConnection: '测试连接',
