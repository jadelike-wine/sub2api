export default {
    ...
    backup: {
      ...
      s3: {
        title: 'S3/R2 Storage Configuration',
        ...
        descriptionSuffix: ')',
        sharedUsageNote: 'This configuration is shared by Agnes multimodal image storage and database backup. Agnes images use the agnes-chat/ prefix, database backups use backups/.',
        enabled: 'Enable S3 Storage',
        endpoint: 'Endpoint',
        region: 'Region',
        bucket: 'Bucket',
        prefix: 'Key Prefix',
        prefixFixedHint: 'Fixed to backups/, not editable. Strictly isolated from image prefixes (agnes-chat/, image-generation/).',
        accessKeyId: 'Access Key ID',
        secretAccessKey: 'Secret Access Key',
        secretConfigured: 'Already configured, leave empty to keep',
        forcePathStyle: 'Force Path Style',
        publicBaseUrl: 'Public Base URL',
        publicBaseUrlHint: 'When a public bucket / CDN domain (e.g. https://cdn.example.com) is set, Agnes images return permanent direct links; leave empty to use expiring presigned URLs. Database backup does not use this field.',
        publicBaseUrlSecurityWarning: 'Security warning: an R2 custom domain / public bucket typically maps the entire bucket. If the domain allows anonymous reads of the backups/ prefix, all database backups will be publicly exposed (backup paths now include a random segment to prevent bulk enumeration, but individual files remain directly accessible). A Cloudflare Worker / CDN auth policy that only allows the agnes-chat/ and image-generation/ prefixes and explicitly denies backups/ is REQUIRED; alternatively keep the bucket private and use presigned URLs. The UI warning is not a security boundary — CDN-level protection is mandatory.',
        additionalPublicBaseUrls: 'Additional Public Base URLs',
        additionalPublicBaseUrlsPlaceholder: 'https://pub-xxx.r2.dev\nhttps://cdn2.example.com',
        additionalPublicBaseUrlsHint: 'If the bucket has an r2.dev Public Development URL or other custom domains, list them one per line. On save, the backend probes each to confirm they also deny anonymous reads of backups/. Recommended: disable these entry points in the Cloudflare dashboard (leave this field empty).',
        bucketPrivacyAttestedLabel: 'I attest the bucket has been privatized (required)',
        bucketPrivacyAttestedHint: 'By checking this, you confirm: R2 Public Development URL (*.r2.dev) is disabled; all custom domains not protected by the Worker have been removed or routed through it; the bucket has no public-read policy; the "Additional Public Base URLs" field above is complete. The backend cannot read Cloudflare control-plane config directly — this attestation is a HARD prerequisite. Also run deploy/cloudflare-worker/verify-policy.mjs for authoritative verification via the Cloudflare API.',
        bucketPrivacyAttestedRequired: 'This attestation is required to save when Public Base URL is set.',
